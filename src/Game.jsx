...
const ETYPES={
  ...
  caster:{hpMul:0.7,spdMul:0.65,szBase:10,armor:1.0,drop:1.4},
};
const TYPE_COL={
  ...
  caster:{r:80, g:160,b:220,dr:70, dg:-20,db:20},
};
const rollType=(wn)=>{const r=Math.random();const tC=wn>=9?0.12:0;const tH=tC+(wn>=12?0.17:0);const tA=tH+(wn>=7?0.18:0);const tS=tA+(wn>=4?0.20:0);if(r<tC)return"caster";if(r<tH)return"healer";if(r<tA)return"armor";if(r<tS)return"swift";return"basic";};
...
const mkRun=(tgt,rlv,equipped,grave)=>{
  ...
  return{tears:(rlv[10]||0)*15,wave:0,tgt,zombies:[],towers:starter,parts:[],projs:[],drops:[],mx:-300,my:-300,
    ...
    kills:0,sQs:[],sel:null,placing:false,lt:0,coreHp:coreBase,coreMax:coreBase,coreReg:(et.core_reg||0),coreShield:Math.min(80,et.core_shield||0),enemyShots:[],
...
export default function Game(){
  ...
  const pad=useRef({on:false,id:null,x:0,y:0});
  ...
  const [padUi,setPadUi]=useState({on:false});
  ...
  useEffect(()=>{const stop=()=>{pad.current.on=false;pad.current.id=null;setPadUi({on:false});};window.addEventListener("blur",stop);return()=>window.removeEventListener("blur",stop);},[]);
  ...
  useEffect(()=>{
    ...
    const loop=now=>{
      ...
      for(let i=s.enemyShots.length-1;i>=0;i--){const e=s.enemyShots[i];e.p+=dt*0.9;if(e.p>=1){if(!bS){const dmg=(6+s.wave*0.22)*(1-s.coreShield/100);s.coreHp-=dmg;if(s.dmgNums.length<30)s.dmgNums.push({x:CX,y:CY-20,v:"-"+Math.floor(dmg),col:"#38bdf8",t:1,big:true});}else if(s.dmgNums.length<30)s.dmgNums.push({x:CX,y:CY-20,v:"SHIELD",col:"#fbbf24",t:1,big:true});s.parts.push({x:CX,y:CY,vx:(Math.random()-0.5)*40,vy:(Math.random()-0.5)*40,life:0.45,col:"#38bdf8",sz:3});s.enemyShots.splice(i,1);if(s.coreHp<=0){s.coreHp=0;s.go=true;endRun(false);}}}
      ...
      for(let i=s.zombies.length-1;i>=0;i--){const z=s.zombies[i];
        ...
        if(z.type==="caster"&&!z.atk){const dx=CX-z.x,dy=CY-z.y,dist=Math.hypot(dx,dy),hold=230;if(dist>hold+8){z.x+=(dx/dist)*z.spd*dt*slowMul;z.y+=(dy/dist)*z.spd*dt*slowMul;}else if(dist<hold-24){z.x-=(dx/dist)*z.spd*dt*0.7*slowMul;z.y-=(dy/dist)*z.spd*dt*0.7*slowMul;}z.castT-=dt;if(z.castT<=0){z.castT=2.2+Math.random()*0.5;s.enemyShots.push({x:z.x,y:z.y,tx:CX,ty:CY,p:0});s.parts.push({x:z.x,y:z.y-8,vx:0,vy:-10,life:0.35,col:"#38bdf8",sz:2.5});}}
        if(!z.atk&&z.type!=="caster"){const dx=CX-z.x,dy=CY-z.y,dist=Math.hypot(dx,dy);if(dist>18){z.x+=(dx/dist)*z.spd*dt*slowMul;z.y+=(dy/dist)*z.spd*dt*slowMul;}else{
      ...
      if(s.enemyShots.length>80)s.enemyShots.splice(0,s.enemyShots.length-80);
  ...
  const padMove=(x,y)=>{const p=pad.current,s=gs.current;if(!s||!cv.current)return;const r=cv.current.getBoundingClientRect();if(s.mx<0||s.my<0){s.mx=CX+80;s.my=CY;}s.mx=Math.max(8,Math.min(W-8,s.mx+(x-p.x)*(W/r.width)));s.my=Math.max(8,Math.min(H-8,s.my+(y-p.y)*(H/r.height)));p.x=x;p.y=y;setPadUi({on:true});};
  const padStart=e=>{e.preventDefault();const t=e.changedTouches[0],s=gs.current;if(s&&(s.mx<0||s.my<0)){s.mx=CX+80;s.my=CY;}pad.current={on:true,id:t.identifier,x:t.clientX,y:t.clientY};setPadUi({on:true});};
  ...
  const padEnd=e=>{if(![...e.changedTouches].some(t=>t.identifier===pad.current.id))return;e.preventDefault();pad.current.on=false;pad.current.id=null;setPadUi({on:false});};
  ...
  return(<div style={{...css.root,position:"relative"}}>
    ...
    {!ui.go&&!ui.vic&&<div
      ...
      style={{height:76,background:padUi.on?"rgba(3,8,9,0.96)":"rgba(3,6,6,0.9)",borderTop:"1px solid rgba(147,197,253,0.12)",borderBottom:"1px solid #162016",boxShadow:"inset 0 14px 22px rgba(0,0,0,0.35), inset 0 0 26px rgba(96,165,250,0.04)",display:"flex",alignItems:"center",justifyContent:"center",touchAction:"none",userSelect:"none"}}>
      <div style={{width:"78%",height:34,borderRadius:6,border:"1px solid rgba(147,197,253,0.14)",background:"rgba(96,165,250,0.035)",boxShadow:padUi.on?"0 0 18px rgba(96,165,250,0.12)":"none"}}/>
    </div>}
