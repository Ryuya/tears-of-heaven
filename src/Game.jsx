...
const mkRun=(tgt,rlv,equipped,grave)=>{
  ...
  return{tears:0,wave:0,tgt,zombies:[],towers:[],parts:[],projs:[],drops:[],mx:-300,my:-300,
    ...
    twSlow:(et.tw_slow||0)/100,curClone:(et.cur_clone||0)/100,clone:null,
  ...
  useEffect(()=>{
    ...
    const loop=now=>{
      ...
      for(let i=s.zombies.length-1;i>=0;i--){const z=s.zombies[i];
        ...
        if(s.clone){const cdC=Math.hypot(z.x-s.clone.x,z.y-s.clone.y);if(cdC<ecR)z.hp-=ecD*dt*25*0.6;}
