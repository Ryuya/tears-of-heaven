...
const computeCombatPower=(rlv,equipped)=>{const et=eqT(equipped);const cD=1+(rlv[1]||0)*0.5+(et.cur_dmg_f||0);const cDp=1+(rlv[1]||0)*0.03+(et.cur_dmg_p||0)/100+(et.all_dmg_p||0)/100;const cR=36+(rlv[0]||0)*4+(et.cur_range||0);const twD=1+(rlv[3]||0)*0.08+(et.tw_dmg_p||0)/100+(et.all_dmg_p||0)/100;const twS=1+(rlv[6]||0)*0.06+(et.tw_spd_p||0)/100;const coreMax=150+(rlv[4]||0)*20+(et.core_hp||0);const coreReg=et.core_reg||0;const coreShield=Math.min(80,et.core_shield||0);const tearP=1+(rlv[2]||0)*0.1+(et.tear_p||0)/100;const crit=(et.crit||0)/100;const waveWipe=(et.wave_wipe||0)/100;const chainKill=(et.chain_kill||0)/100;const cursorPot=cD*cDp*25*60*(cR/36)*(1+crit*4);const towerPot=17*5*twD*twS*60;const survBudget=(coreMax+coreReg*30)*(1+coreShield/100);const wipeBonus=waveWipe*500000;const chainMul=1+chainKill*0.5;return Math.round((cursorPot+towerPot+survBudget+wipeBonus)*chainMul*tearP);};
